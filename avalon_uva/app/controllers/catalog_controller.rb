...
class CatalogController < ApplicationController
  ...
  rescue_from Blacklight::Exceptions::InvalidRequest do |exception|
    if exception.message.include? "too many boolean clauses"
      if user_session.try(:[], :virtual_groups).present? && current_user.present?
        Rails.logger.warn "Too many boolean clauses caused by #{current_user.try :username}'s #{user_session[:virtual_groups].count} groups. Truncating"
        user_session[:virtual_groups] = user_session[:virtual_groups][..50]
