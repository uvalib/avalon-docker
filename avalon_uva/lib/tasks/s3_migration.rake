    ...
    namespace :masterfiles do
      ...
      task :from_lib_content => :environment do
        ...
        mfSolr["response"]["docs"].each do |mf|
          if mf['file_location_ssi'][/\/lib_content60/] # 226 records lib_content60 became lib_content114
          ...
          if mf['file_location_ssi'][/\/lib_content105/] # 192 records, became lib_content112
          ...
          if mf['file_location_ssi'][/\/lib_content67/] # 1216 records, became lib_content112
          ...
          if mf['file_location_ssi'][/\/lib_content112/] # fix s3 path
          ...
          when /\/lib_content114/, /\/lib_content122/
            #puts old_path
            next
          ...
          when /s3:\/\/content112/
            ...
            if !dest_object.exists?
              if source_object.restore && source_object.restore.include?('ongoing-request="true"')
                @pending_restore << mf
              elsif source_object.restore && source_object.restore.include?('expiry-date')
                # Has been restored
                # Now copy to the destination
                if s3_copy(source_object, dest_object)
                  @copied << mf
                end

              elsif source_object.storage_class.include?("GLACIER")
                if send_s3_restore(source_object)
                  @restore_sent << mf
                end
              else
                byebug
      ...
      def new_s3_path(id, old_path)
        ...
        return File.join(
          s3_base_uri,
          MasterFile.post_processing_move_filename(old_path, id: id)
        )
      end

      def cardinal_check(old_path)
        out = %x(ssh -Sv naw4t@cardinal.lib.virginia.edu stat -c%s "#{old_path.shellescape}")
        return out.to_i > 0 ? out.to_i : false
      end
