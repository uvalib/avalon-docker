...
FROM        ruby:3.4.8-bookworm AS bundle
LABEL       stage=build
...
COPY        avalon_uva/bin/yaz-config ./bin/yaz-config
...
FROM        ruby:3.4.8-bookworm AS download
LABEL       stage=build
LABEL       project=avalon
...
RUN         mkdir -p /home/app/avalon/tmp/cache /home/app/avalon/node_modules
RUN         chown app:app -R /home/app/avalon

COPY        active_encode_uva/ffmpeg_adapter.rb /usr/local/bundle/gems/active_encode-1.3.0/lib/active_encode/engine_adapters/ffmpeg_adapter.rb
RUN         chown app:app /usr/local/bundle/gems/active_encode-1.3.0/lib/active_encode/engine_adapters/ffmpeg_adapter.rb
...
# If bundle install needs to be run here, the container probably has an old Gemfile.lock.
...
COPY        --from=bundle-prod --chown=app:app /usr/local/bundle /usr/local/bundle
COPY        --chown=app:app active_encode_uva/ffmpeg_adapter.rb /usr/local/bundle/gems/active_encode-1.3.0/lib/active_encode/engine_adapters/ffmpeg_adapter.rb
